...
echo "[demo] step 5: start mcpd"
run_as_user bash scripts/run_mcpd.sh

echo "[demo] step 6: llm-app once: hello"
run_as_user python3 llm-app/cli.py --once "hello"

echo "[demo] step 7: llm-app once: burn cpu for a bit"
run_as_user python3 llm-app/cli.py --once "burn cpu for a bit"

echo "[demo] step 8: verify sysfs agent completion counters"
...
echo "[demo] step 9: stop mcpd"
run_as_user bash scripts/stop_mcpd.sh

echo "[demo] step 10: unload module"
${SUDO} bash scripts/unload_module.sh

echo "[demo] step 11: reload_10x"
