...
\begin{figure}[t]
\centering
\includegraphics[width=0.75\linewidth]{figures/figure_boundary_matrix.pdf}
\caption{Boundary-condition supplement visualised. Each row is one of the eight probe cases (the race probe appears as two rows, one per concurrent worker), each column is a system, and each cell shows the absolute count (successes/attempts) of attempts that bypassed the invariant. Every cell is 0/10 across all three systems; the colorbar axis is kept at its full [0, 1] range so that this result is visually comparable to the main attack matrix in Figure~\ref{fig:chapter4-security}.}
\label{fig:chapter4-boundary}
\end{figure}
