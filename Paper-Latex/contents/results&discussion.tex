...
Neither difference is statistically significant at $\alpha = 0.05$ (Welch's $t$-test, $p = 0.526$ and $p = 0.287$ respectively), which is consistent with the expectation that the fixed cost of a Generic Netlink round-trip is small relative to the remaining user-space path.
...
The reconciliation path works as follows. On startup, \texttt{mcpd} queries the kernel's current tool registry via \texttt{LIST\_TOOLS\_DUMP}, compares the returned hashes against those computed from the manifests on disk, and issues \texttt{TOOL\_REGISTER} messages only for tools whose hash is absent or stale. This is a conservative strategy---it rebuilds from the manifest source of truth rather than trusting that partial kernel state is still valid---but it keeps the recovery logic straightforward and avoids the need to reason about partial updates.
