...
\caption{Mean end-to-end latency by payload size for the three systems. Each subplot auto-scales its own y-axis so that the sub-millisecond differences at 100 B and 10 KiB are not flattened by the much larger absolute values at 1 MiB. Error bars show 95\% confidence intervals across $n = 30$ independent runs; significance markers are Welch's $t$-test against the \texttt{userspace} baseline (\texttt{ns} = not significant, * = $p < 0.05$, ** = $p < 0.01$, *** = $p < 0.001$).}
...
\caption{Mean-latency ratio of each hardened variant to the plain \texttt{userspace} baseline, with 95\% confidence intervals propagated from the 30-run per-repetition means. The y-axis is zoomed around the parity line (ratio = 1.00) to make the sub-4\% effects at 100 B and 10 KiB visible alongside the larger effect at 1 MiB. The \texttt{seccomp} path is significantly slower at every payload size, while the \texttt{kernel} path is indistinguishable from \texttt{userspace} at small/medium payloads and measurably faster at 1 MiB.}
...
\caption{Invocation stage breakdown (session lookup, arbitration, tool execution) for each system at all three payload sizes, on a log y-axis. The log scale makes it possible to see session\_lookup (around 4\,\textmu s) at the same time as tool\_exec (around 4\,ms). The \texttt{kernel} path spends roughly one order of magnitude more time in arbitration than the user-space baselines, but that extra cost is well under \num{0.04}\,ms and is invisible relative to the tool-execution portion of the invocation.}
...
To assess whether the kernel-assisted control plane becomes a bottleneck under load, throughput was measured across a range of agent counts (1, 5, 10, 20, 50) and concurrency levels (1, 10, 50, 100).
...
\begin{figure}[t]
\centering
\includegraphics[width=0.75\linewidth]{figures/figure_attack_matrix.pdf}
\caption{Attack bypass rate by category and system. Each cell shows the absolute count (successes/attempts) and the percentage bypass rate; darker cells indicate a higher bypass rate. The \texttt{kernel} column is uniformly zero across all four categories, while the two user-space baselines still leak on the specific checks that their instrumented tamper profiles relax.}
...
As a result, a 0\% bypass rate on the kernel path should not be read as evidence of adversarial robustness against unknown attacks; it is a functional-correctness test of four specified invariants against inputs designed to exercise exactly those invariants.
The test set confirms that the enforcement logic behaves as specified on the cases its author anticipated, and makes no claim beyond that.
...
All eight cases are rejected on all three systems across all ten repeats (240 attempts total, zero bypasses). Three aspects of this result deserve honest recording rather than a uniform win.
...
Each of the four experiments probes a different facet of the same underlying question: is kernel placement of control-plane state consequential, and at what cost?

The security evaluation gives the clearest answer. The kernel path blocks all 110 attack attempts across four categories; neither user-space baseline can do the same, even when hardened with stricter syscall filtering. The reason is structural rather than quantitative: approval tickets, agent bindings, and semantic hashes are held in kernel memory under \texttt{CAP\_NET\_ADMIN}-enforced access control, so a user-space process cannot modify or bypass them without a privilege escalation that is out of scope for the stated threat model. The seccomp baseline's 36.4\% bypass rate illustrates exactly what user-space hardening cannot achieve: it reduces the syscall surface but leaves the admission decision itself---and the state it depends on---in a mutable process.

The performance results show that this structural improvement does not come at a prohibitive cost. The most informative comparison is at large payloads: the kernel path is \emph{faster} than the seccomp baseline at 1 MB because BPF-based per-syscall filtering accumulates cost with payload size, while kernel arbitration does not. The naive expectation---that any additional privilege boundary must slow the path down---does not hold here. The more productive framing is not ``kernel or user space?'' but ``what kind of enforcement is being performed, and where is its cost incurred?''

The failure experiment reveals a dimension that performance and security benchmarks cannot capture: durability. In both user-space baselines, daemon failure is equivalent to control-plane failure---approval tickets and registry state vanish. Approval state persists across failure in the kernel path and remains inspectable via \texttt{sysfs} without any daemon involvement. This property cannot be confirmed by a benchmark; it follows directly from holding that state in the kernel rather than in a process-local data structure, and the recovery experiment confirms it by observation.

The main open question is behaviour at substantially higher concurrency. The current experiments reach 100 concurrent requests without evidence of a kernel-side bottleneck, but the registry lock and ticket-management logic have not been tested under workloads with many simultaneously deferred approvals. Whether that becomes a bottleneck---and whether the lock could be decomposed to reduce contention---is a natural direction for follow-on work.


\subsection{Threats to Validity}

Several limitations bear on what the numbers in this chapter can and cannot support.

\paragraph{Statistical power of the latency comparison.} All latency experiments were conducted on a single VMware guest on an Apple Silicon host; absolute numbers are specific to that environment, and hypervisor scheduling noise cannot be fully eliminated from userland measurements. At 100 B and 10 KB, the point-estimate difference between the kernel path and the user-space baseline (\num{0.00}--\num{0.04} ms) is within the per-run standard deviation (\num{0.04}--\num{0.10} ms). The $n = 30$ rerun reduces the standard error of the mean relative to the earlier pilot but is not sufficient power to distinguish differences inside the hypervisor noise floor. The $p$-values at 100 B and 10 KB ($p = 0.984$ and $p = 0.097$) indicate that any kernel-side overhead at those sizes is below the measurement threshold in this environment; they do not constitute evidence of true equivalence. The 1 MB result is different in kind: the difference (\num{0.26} ms) exceeds the per-run standard deviation (\num{0.29} ms) and the $t$-test reaches $p = 0.00027$. A definitive small-payload comparison would require bare-metal evaluation, which is outside the scope of this dissertation.
