...
The main outcome of the project is not simply a working prototype, but a more precise architectural argument. Rather than treating AI-agent tool use purely as an application-layer integration problem, \texttt{linux-mcp} shows that a small but consequential part of the control plane benefits from operating-system-level support. 
Admission decisions, approval-ticket state, binding checks, and authoritative registry state all gain from being placed in a stronger enforcement domain than a mutable user-space mediator can provide. 
By contrast, semantic parsing, schema validation, session logic, and tool execution remain in user space, where flexibility and evolvability are more important than privilege.

This split is the central result of the project. It shows that stronger control does not require relocating the entire agent stack into the kernel. 
A narrow kernel role is sufficient to make the most critical state transitions---semantic identity, requester binding, approval validity, and invocation completion---explicit, authoritative, and independently inspectable.
In this sense, \texttt{linux-mcp} does not argue for a kernel-heavy AI operating system; it argues for careful placement of a small set of control-plane invariants.
