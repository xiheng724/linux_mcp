...
This motivates a focused question: which control-plane invariants in tool invocation fundamentally benefit from kernel-level enforcement, and can such enforcement improve security and resilience without sacrificing performance?
...
This design does not reduce the trusted footprint of \texttt{mcpd}---the gateway remains fully trusted and populates the kernel's registries.
...
Security model
& TCB partitioned, not reduced
& Monolithic gateway state \\
...
\begin{itemize}
    ...
    \item We conduct a controlled functional evaluation against pure and hardened user-space baselines across latency, throughput, attack resistance, and failure scenarios. The evaluation confirms that \texttt{linux-mcp} correctly enforces the specified control-plane invariants against a category-level attack set and against a supplementary boundary-condition probe, characterises the latency cost of kernel placement at $n = 30$, and shows that kernel-held approval state survives daemon failure in a way neither user-space baseline can match. We explicitly frame this as a functional-correctness evaluation of specified invariants rather than as an adversarial-robustness evaluation against unknown attacks.
\end{itemize}
