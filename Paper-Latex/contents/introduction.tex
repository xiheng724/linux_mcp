...
\begin{itemize}
    ...
    \item We conduct a controlled functional evaluation against pure and hardened user-space baselines across latency, throughput, attack resistance, and failure scenarios. The evaluation confirms that \texttt{linux-mcp} correctly enforces the specified control-plane invariants against a category-level attack set (0 of 110 kernel-path attempts succeed, versus 90.9\% and 36.4\% against the two instrumented user-space baselines) and against a supplementary boundary-condition probe (0 of 240 additional attempts), characterises the latency cost of kernel placement at $n = 30$, and shows that kernel-held approval state survives daemon failure in a way neither user-space baseline can match. We explicitly frame this as a functional-correctness evaluation of specified invariants rather than as an adversarial-robustness evaluation against unknown attacks.
\end{itemize}
