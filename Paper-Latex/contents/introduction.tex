...
Semantic consistency is enforced through manifest-derived identifiers, identity binding is anchored in OS-level peer credentials, and approval state is maintained in the kernel across daemon failures. 
All tool invocations therefore pass through a single mediation path, reducing reliance on in-memory gateway state.
...
\begin{itemize}
    ...
    \item We design and implement \texttt{linux-mcp}, a kernel-assisted architecture that enforces these invariants through semantic-hash binding, Unix-domain credential binding, and a kernel-managed approval workflow resilient to daemon failure.
    
    \item We formulate kernel-assisted control of tool invocation as a systems problem for AI agent workloads on Linux, identifying a mismatch between existing operating system abstractions and agent-centric execution models.
    
    \item We conduct a comprehensive evaluation against pure and hardened user-space baselines across latency, throughput, attack resistance, and failure scenarios, showing that \texttt{linux-mcp} eliminates all tested attack classes with negligible overhead.
\end{itemize}
