...
Table~\ref{tab:netlink-attrs-full} lists the attributes exercised on the admission-critical \texttt{TOOL\_\allowbreak REQUEST}/\texttt{TOOL\_\allowbreak DECISION} and approval paths.
The full family also defines auxiliary attributes for registration, status reporting, payload/response audit hashing, and epoch bookkeeping; the subset reproduced inline in Section~\ref{sec:wire-protocols} is a further reduction of the request-critical fields shown here.
...
\bottomrule
\end{tabular}
\caption{Core \texttt{KERNEL\_\allowbreak MCP} TLV attributes on the admission path. All strings are NUL-terminated and 4-byte aligned. Registration and reconciliation paths additionally carry a \texttt{CATALOG\_\allowbreak EPOCH} attribute (id 34) that advances when a tool's manifest is re-registered; \texttt{mcpd} reads it back after each \texttt{TOOL\_REGISTER} to compare against a session's \texttt{opened\_\allowbreak at\_\allowbreak epoch}.}
\label{tab:netlink-attrs-full}
\end{table}

\paragraph{Semantic hash computation.}

The semantic hash used for tool identity is a full 256-bit SHA-256 digest computed over a canonical JSON serialisation of the eleven manifest fields enumerated in the \texttt{SEMANTIC\_FIELDS} array of the listing below.
...
SEMANTIC_FIELDS = [
    ...
    "operation",   # in-hash: repoints the RPC method executed on the backend
]
...
\texttt{approval\_policy} & \ding{51} & Object specifying when user confirmation is required (e.g., \texttt{\{"user\_\allowbreak confirmation":\allowbreak{} \{"when":\allowbreak{} "always"\}\}}); defaults to \texttt{\{\}} for unrestricted tools \\
\rowcolor{gray!8}
...
\paragraph{Noise-floor command: \texttt{KERNEL\_MCP\_CMD\_NOOP}.}
...
The command is appended to the existing \texttt{KERNEL\_\allowbreak MCP} family at a stable numeric ID so that older \texttt{mcpd} builds continue to work unchanged:
...
The \texttt{SKIP\_*} flags are honoured only under an experiment-enabled build (\texttt{CONFIG\_KERNEL\_MCP\_}\allowbreak\texttt{EXPERIMENT=y}); production builds ignore the attribute entirely, so the mere presence of the UAPI constants does not weaken the enforcement path.
...
\setlength{\tabcolsep}{6pt}
\begin{tabular}{p{3.3cm} p{3.1cm} p{7.3cm}}
\toprule
\textbf{Artefact} & \textbf{Exposed as} & \textbf{Purpose} \\
\midrule
\rowcolor{gray!8}
\texttt{KERNEL\_\allowbreak MCP\_\allowbreak CMD\_\allowbreak NOOP} & UAPI command (\#15) & Generic Netlink round-trip noise-floor measurement. Returns immediately after dispatch; used by the kernel-ablation runner to anchor μs-level claims. \\
\texttt{SKIP\_LOOKUPS} & \texttt{EXPERIMENT\_\allowbreak FLAGS} bit 0 & Bypasses the tool and agent registry walks. Used as the \texttt{skip\_lookups} bar in Figure~\ref{fig:chapter4-ablation} and as the bypass mode in the paired registry+lookup delta in Table~\ref{tab:ablation-stages}. \\
\rowcolor{gray!8}
\texttt{SKIP\_HASH} & \texttt{EXPERIMENT\_\allowbreak FLAGS} bit 1 & Bypasses the hash-mismatch guard. Guard body is dead under benign credentials; retained as a paired consistency check. \\
\texttt{SKIP\_BINDING} & \texttt{EXPERIMENT\_\allowbreak FLAGS} bit 2 & Bypasses the binding-mismatch guard. Also dead under benign credentials; paired consistency check. \\
\rowcolor{gray!8}
\texttt{SKIP\_TICKET} & \texttt{EXPERIMENT\_\allowbreak FLAGS} bit 3 & Bypasses the approval-ticket issuance and consumption branch. Used as the \texttt{ticket\_trigger\_skip} mode to isolate the approval-ticket body cost. \\
\texttt{require\_\allowbreak peer\_\allowbreak cred} & Module parameter (\texttt{bool}, 0644) & Enables the \texttt{NETLINK\_CB} peer-UID check in \texttt{kernel\_mcp\_\allowbreak cmd\_\allowbreak tool\_\allowbreak request}. Default \texttt{false} so that the cross-UID experiment can toggle it at runtime; should be set to \texttt{true} in production. \\
\bottomrule
\end{tabular}
