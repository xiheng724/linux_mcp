...
An agent plans over semantically described tools, constructs structured requests, and acts on behalf of a user. 
Correctness therefore depends not only on process isolation, but also on whether the requested semantics match the operation that is actually executed.
...
Existing kernel-native mechanisms such as seccomp-BPF, eBPF/LSM hooks, Landlock, and capability-based designs provide strong control over syscalls, kernel objects, and process-scoped authority~\cite{seccomp2026, lsm2026, landlock2026, watson2010capsicum, brimhall2023mac, lefeuvre2024sok}.
...
Industrial deployments such as Harness~\cite{harness2025} add platform-level Role-Based Access Control (RBAC) on top, but the trust anchor is relocated to a cloud control plane rather than eliminated.
Recent work on application-layer mandatory access control including privilege-escalation frameworks for LLM agents~\cite{ji2026seagent} and programmable privilege control for tool-calling agents~\cite{shi2025progent} have enforced policy inside a modified agent runtime, so invariant correctness depends on the runtime itself remaining intact.
...
\toprule
\textbf{Attack class} & \textbf{Representative attack} & \textbf{Defence in \texttt{linux-mcp}} \\
\midrule

Spoofing
& Forged session or tool hash; same-UID session reuse
& \texttt{SO\_\allowbreak PEERCRED} binding hash over $(\text{uid},\text{gid},\text{pid},\text{session\_id})$; kernel-side hash check \\

Replay
& Stale ticket or session reused across manifest change
& Ticket bound to \texttt{(binding\_\allowbreak hash, binding\_\allowbreak epoch)} with TTL; per-tool epoch forces rebind on re-registration \\

Substitution
& Tool diverges from declared semantics, or backend binary swapped behind a live socket
& Kernel-verified manifest hash; TOFU \texttt{binary\_\allowbreak hash} over \texttt{/\allowbreak proc/\allowbreak <pid>/\allowbreak exe} (composite digest for interpreters) \\

Escalation
& Gateway logic bypass
& Mandatory kernel arbitration on every invocation \\

Audit loss
& Kill \texttt{mcpd} to erase recent history
& Fixed-size per-agent \texttt{call\_log} in \texttt{sysfs}, survives daemon crash \\

\bottomrule
\end{tabular}
\caption{Control-plane threats and defences (in scope). The last row is an observability property rather than a classical attack class.}
...
The attacker interacts exclusively through the \texttt{mcpd} JSON-RPC surface and holds no elevated capabilities: specifically, no \texttt{CAP\_\allowbreak NET\_\allowbreak ADMIN} (required to invoke the kernel module via Netlink), no \texttt{ptrace} access to \texttt{mcpd}, and no write access to manifest files.
These three boundaries are critical: \texttt{CAP\_\allowbreak NET\_\allowbreak ADMIN} is the interface to kernel enforcement; \texttt{ptrace} would allow process manipulation; manifest write would break the trusted-input assumption.
...
\emph{Supply-chain compromise of manifests} requires file-system-level defences such as Integrity Measurement Architecture or device-mapper verity; \texttt{linux-mcp} treats on-disk manifests as trusted input and assumes the adversary lacks write access to \texttt{tool-app/\allowbreak manifests/}.
Also excluded from our scope are process-level attacks on the gateway, including ptrace, memory corruption, and \texttt{LD\_\allowbreak PRELOAD}, as well as kernel-level attacks such as rootkits and hardware side channels, and remote adversaries.
Trust in \texttt{mcpd} is time-partitioned rather than absolute.
At initialisation, the daemon is trusted to register manifest hashes and bind sessions to peer credentials; this is the one phase where kernel state depends on daemon correctness.
At run time, no such trust is assumed: every \texttt{TOOL\_\allowbreak REQUEST} and \texttt{APPROVAL\_\allowbreak DECIDE} is re-checked against kernel-resident registry and ticket state, so the placement defends against (a) daemon logic bugs such as session-id mismatch, missed ticket transitions, or concurrent TOCTOU, and (b) unprivileged co-located adversaries observing or forging JSON-RPC traffic.
It still does not protect against a gateway whose process memory has been directly compromised during the initialisation phase or otherwise.
...
A fifth property, \emph{post-compromise audit continuity}, is orthogonal to these four: a gateway crash should not erase the record of recent activity.
...
Their mechanisms are described in Chapter~\ref{chap:design} and exercised adversarially in \hyperref[chap:evaluation]{Chapter~4}.
...
The supplied systemd unit (Section~\ref{sec:daemon-implementation}) runs \texttt{mcpd} as an unprivileged user with only \texttt{CAP\_\allowbreak NET\_\allowbreak ADMIN} and \texttt{CAP\_\allowbreak SYS\_\allowbreak PTRACE}, as defence-in-depth rather than part of the model itself.
