...
& \texttt{SO\_PEERCRED} binding hash over $(\text{uid},\text{gid},\text{pid},\text{session\_id})$; kernel-side hash check \\

Replay
& Stale ticket or session reused across manifest change
& Ticket bound to \texttt{(binding\_hash, binding\_epoch)} with TTL; per-tool epoch forces rebind on re-registration \\

Substitution
& Tool diverges from declared semantics, or backend binary swapped behind a live socket
& Kernel-verified manifest hash; TOFU \texttt{binary\_hash} over \texttt{/proc/<pid>/exe} (composite digest for interpreters) \\
...
\emph{Supply-chain compromise of manifests} requires file-system-level defences such as Integrity Measurement Architecture or device-mapper verity; \texttt{linux-mcp} treats on-disk manifests as trusted input and assumes the adversary lacks write access to \texttt{tool-app/manifests/}.
Also excluded from our scope are process-level attacks on the gateway, including ptrace, memory corruption, and \texttt{LD\_PRELOAD}, as well as kernel-level attacks such as rootkits and hardware side channels, and remote adversaries.
...
The supplied systemd unit (Section~\ref{sec:daemon-implementation}) runs \texttt{mcpd} as an unprivileged user with only \texttt{CAP\_NET\_ADMIN} and \texttt{CAP\_SYS\_PTRACE}, as defence-in-depth rather than part of the model itself.
