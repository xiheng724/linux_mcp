...
Hot reload is request-driven rather than filesystem-driven: \texttt{mcpd} recomputes a manifest-set signature on the \texttt{list\_apps}, \texttt{list\_tools}, and \texttt{tool:exec} paths and refreshes the kernel registry when the signature changes, so a manifest edit takes effect on the next request rather than the instant the file is written.
