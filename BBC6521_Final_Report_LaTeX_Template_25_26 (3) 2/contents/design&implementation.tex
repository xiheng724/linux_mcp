...
From these the daemon derives a binding hash $\operatorname{BLAKE2b}(\text{uid}, \text{gid}, \text{pid}, \text{session\_id})$ and a binding epoch; including \texttt{pid} and a per-session random \texttt{session\_id} means that two processes sharing a UID still cannot reuse each other's session.
...
On \texttt{APPROVAL\_DECIDE}, it checks that the ticket's binding still matches the current session before letting the request proceed.
...
The kernel therefore maintains a global \texttt{tool\_catalog\_epoch} counter, incremented on every (un)registration; each tool records its \texttt{registered\_at\_epoch}, and each session its \texttt{opened\_at\_epoch}.
If a \texttt{TOOL\_REQUEST} targets a tool re-registered after the session opened, it is denied with \texttt{catalog\_stale\_rebind\_required} and the client must open a new session.
...
The kernel therefore keeps a per-tool \texttt{binary\_hash} slot populated on first use: \texttt{mcpd} reads the backend's peer PID via \texttt{SO\_PEERCRED} and hashes \texttt{/proc/<pid>/exe}, using a composite digest over interpreter plus script for interpreter-hosted tools so a script swap is still caught.
The digest is attached to the first \texttt{TOOL\_REGISTER}; later registrations or requests carrying a different digest are rejected.
A \texttt{binary\_hash\_state} field (\texttt{unpinned} or \texttt{live\_pinned}) distinguishes ``no probe yet'' from ``pinned'', since both previously read as an empty string.
...
Records are written on both the \texttt{ALLOW}+\texttt{TOOL\_COMPLETE} and \texttt{DENY}/\texttt{DEFER} paths, so successful, rejected, and deferred invocations are all captured.
...
Under \texttt{/sys/kernel/mcp/tools/}, each tool directory exposes \texttt{name}, semantic \texttt{hash}, \texttt{binary\_hash}, \texttt{binary\_hash\_state}, and \texttt{registered\_at\_epoch}; the global \texttt{tool\_catalog\_epoch} sits at the top of the tree.
Under \texttt{/sys/kernel/mcp/agents/}, per-agent entries expose \texttt{allow}/\texttt{defer}/\texttt{completed\_ok}/\texttt{last\_exec\_ms} counters, a human-readable \texttt{last\_reason}, and the \texttt{call\_log} above with its head and count indices.
...
\texttt{mcpd} needs \texttt{CAP\_NET\_ADMIN} for Netlink administration and \texttt{CAP\_SYS\_PTRACE} for reading \texttt{/proc/<pid>/exe} across uid boundaries during the backend probe.
The supplied systemd unit grants both via \texttt{AmbientCapabilities} and runs the daemon as an unprivileged \texttt{mcpd} user with \texttt{NoNewPrivileges} and a read-only bind mount of the manifest directory, so gateway RCE cannot load kernel modules or mutate manifests.

A separate question is which UIDs the daemon accepts as tool backends.
\texttt{allowed\_backend\_uids} is checked via \texttt{SO\_PEERCRED} on both probe and exec dials.
An unprivileged \texttt{mcpd} defaults to its own UID.
A privileged \texttt{mcpd} has no safe default: trusting only UID~0 would reject every realistic non-root backend and leave \texttt{binary\_hash} unpinned, while trusting \texttt{\$SUDO\_UID} would fold launcher-script state into the daemon's trust boundary.
The daemon therefore refuses to start unless the list is declared in the TOML or the operator explicitly opts in to \texttt{\$SUDO\_UID} trust via an environment variable.
...
At cold start, \texttt{mcpd} issues \texttt{RESET\_TOOLS} and re-registers the full manifest set, rebuilding kernel state from scratch rather than trying to decide whether residual state is still valid.
While the daemon is running, manifest changes are applied incrementally: only mutated tools re-register, and each such call advances both the tool's \texttt{registered\_at\_epoch} and the global \texttt{tool\_catalog\_epoch}.
If the backend's peer UID or binary digest has changed, the kernel TOFU slot is cleared before re-registration so the new digest can be pinned without inheriting the stale one.
Together with the epoch-driven rebind described earlier, this gives a real hot-reload path: only the affected tools re-register, and only the sessions that actually referenced them are forced to rebind.

Session state is lost on crash, since the binding context for in-progress sessions exists only in the daemon process; approval tickets, registry state, and the per-agent \texttt{call\_log} held in the kernel are not.
...
Cold-start reconciliation is unconditional: every launch pays a \texttt{RESET\_TOOLS} plus a full \texttt{TOOL\_REGISTER} sweep, even when manifests are unchanged.
Hot reload is supported at run time but not wired to \texttt{inotify}, so a manifest edit on disk needs an explicit signal into \texttt{mcpd}.
Approval tickets carry a five-minute deadline but are not actively swept; expired tickets linger until touched.
