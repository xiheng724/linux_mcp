...
A supplementary microbenchmark isolates the Generic Netlink layer using a minimum-cost \texttt{KMCP\_CMD\_NOOP} handler added to the kernel module.
...
Figure~\ref{fig:chapter4-ablation} reports the medians on a single axis: the NOOP floor sits at \num{3.125}~$\mu$s, the full \texttt{TOOL\_REQUEST} path measured at a realistic registry scale of 1024 tools and 64 agents sits at \num{7.459}~$\mu$s, and the \texttt{skip\_lookups} and \texttt{ticket\_trigger} bars bracket the per-stage contributions that the next paragraph decomposes.
...
\caption{Kernel path ablation at realistic registry scale ($N = 1024$ tools, $M = 64$ agents). The leftmost bar is the measured noise floor from the \texttt{KMCP\_CMD\_NOOP} handler, which establishes the irreducible Generic Netlink round-trip cost. The \texttt{full} bar is the complete \texttt{TOOL\_REQUEST} arbitration path; \texttt{skip\_lookups} disables the tool and agent registry walks. The two \texttt{ticket\_trigger} bars exercise a high-risk tool whose execution is gated by the approval-ticket machinery, with and without the approval branch bypassed. All error bars are 95\% bootstrap CIs across $n = 10$ repetitions.}
\label{fig:chapter4-ablation}
\end{figure}

To decompose the \num{4.334}~$\mu$s arbitration cost into its constituent stages, a paired ablation was performed against compile-time experiment flags that selectively bypass individual kernel code paths (\texttt{SKIP\_LOOKUPS}, \texttt{SKIP\_HASH}, \texttt{SKIP\_BINDING}, \texttt{SKIP\_TICKET}).
...
The \texttt{registry+agent\_lookup} stage (two mutex pairs, an \texttt{xa\_load} against a 1024-entry xarray, and a 64-entry hashtable walk) contributes \num{0.109}~$\mu$s per request.
The \texttt{approval\_ticket\_body} stage (consume plus issue, including an \texttt{approval\_lock} acquisition and hashtable insert) contributes \num{2.012}~$\mu$s per request and is only exercised when the tool is high-risk and the request arrives without a pre-authorised ticket.
...
The remainder is reply-skb construction: the five \texttt{nla\_put\_*} calls and the \texttt{genlmsg\_reply} in \texttt{kernel\_mcp\_reply\_tool\_decision}, which the experiment flags do not decompose further.

One caveat about the \texttt{approval\_ticket\_body} figure deserves explicit mention.
Every ticket issuance calls \texttt{kernel\_mcp\_purge\_expired\_tickets\_locked()}, which iterates the full 256-bucket approval hashtable.
...
Moving the expiry scan to the existing periodic \texttt{kernel\_mcp\_ticket\_cleanup\_timer} would eliminate this $O(n)$ component entirely.
...
A separate experiment targets three threat classes that sit outside that set and that a design-time review would not surface: a time-of-check-time-of-use (TOCTOU) race on the approval window, a cross-UID session hijack under adversarial Generic Netlink credentials, and dumb fuzzing of the \texttt{KERNEL\_MCP} Generic Netlink family.
...
Across \num{10000} iterations, zero \texttt{ALLOW} outcomes were produced: \num{9985} requests were rejected with \texttt{hash\_mismatch} at the kernel boundary, and the remaining 15 were deferred with \texttt{approval\_ticket\_unknown} because the consume attempt raced ahead of ticket issuance.
The Clopper–Pearson 95\% CI on the blocked rate is $[0.9996, 1.0000]$.
The absence of breaches is consistent with the lock ordering in \texttt{kernel\_mcp\_cmd\_tool\_request}: the ticket consume and the tool-hash recomputation both acquire \texttt{tool\_lock}, so a replacement that swaps the underlying binding cannot land between the two checks.
...
In the version merged from the main evaluation branch, \texttt{kernel\_mcp\_cmd\_tool\_request} never compares \texttt{NETLINK\_CB(skb)} peer credentials against the \texttt{uid} recorded at agent registration, so a crafted-credential replay from UID $B$ reaches the request handler as if it had been sent by UID $A$.
...
The fix is a six-line patch that walks \texttt{sock\_i\_uid(NETLINK\_CB(skb).sk)} on entry to the handler and rejects the request with \texttt{-EPERM} on mismatch, gated behind a new \texttt{require\_peer\_cred} module parameter so that the A/B comparison can be run against a single binary.
...
The patch adds no measurable latency because the peer-credential read is on the same cache line as the existing \texttt{NETLINK\_CB} access that the handler already performs.
...
The \texttt{KERNEL\_MCP} family was exercised with a coverage-free structured fuzzer that iterates every defined \texttt{KMCP\_CMD\_*} opcode and applies per-\texttt{nlattr} bit flipping, length mutations, and type substitutions.
...
\caption{Errno distribution for the Generic Netlink dumb fuzzer, binned by the \texttt{KMCP\_CMD\_*} opcode under test. Every rejection falls into one of three deterministic errno classes (\texttt{-EINVAL}, \texttt{-EBADMSG}, \texttt{-EPERM}); zero mutated messages produced a kernel oops, \texttt{WARN}, or \texttt{kmemleak} report across \num{880640} total inputs.}
...
The extended-attack results introduce one piece of genuine bad news and one genuine good one.
The bad news is that the cross-UID experiment surfaced a missing peer-credential check in the stock binary — exactly the kind of invariant a kernel-held control plane is supposed to enforce, and exactly the kind of defect a design-time review is unlikely to catch because the offending code is an absence rather than a presence.
The good news is that the fix is six lines, costs no measurable latency, and closes the A/B gap completely.
That kind of finding is the reason for running the extended-attack rerun at all: the design-time threat model and the category-level harness both pointed at 0\% bypass, and only a different experiment with a different premise was able to surface the hole.
For any future kernel-assisted control plane the lesson is cheap — a peer-credential compare on entry to every privileged command handler, added at registration, not as an afterthought.
