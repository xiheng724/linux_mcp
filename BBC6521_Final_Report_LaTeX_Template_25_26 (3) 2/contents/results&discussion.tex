...
\caption{Latency summary across the three systems at 100~B, 10~KiB, and 1~MiB payloads ($n = 30$ independent runs of 2000 requests, error bars 95\% CI). Panel (a): mean end-to-end latency on a log y-axis so sub-millisecond gaps at small payloads remain visible alongside the 1~MiB values. Panel (b): $p_{95}$ latency; the three systems are indistinguishable at 100~B and 10~KiB, and the kernel $p_{95}$ sits below both baselines at 1~MiB. Panel (c): mean-latency ratio against the \texttt{userspace} baseline, zoomed around the parity line; \texttt{seccomp} sits above parity at every payload size, while the kernel path only crosses below parity at 1~MiB. Panel (d): control-plane stage zoom in $\mu$s; session lookup is comparable across systems, and kernel arbitration adds roughly $+26.7\,\mu$s above the userspace path but is still two orders of magnitude below tool execution.}
...
No heavy right tail appears on the kernel path at any payload size, which is the distributional counterpart of the $p_{95}$ result.
...
\midrule
\rowcolor{red!10}
\texttt{userspace} & 100 & 110 & $90.9\%$ & $[85.1,\,95.7]$ \\
\rowcolor{orange!10}
\texttt{seccomp}   &  40 & 110 & $36.4\%$ & $[28.4,\,45.8]$ \\
\rowcolor{green!10}
\texttt{kernel}    &   0 & 110 & $\mathbf{0.0\%}$ & $[0.0,\,3.3]$ \\
\bottomrule
\end{tabular}
\end{table}
...
It still admits a substantial fraction of the attack attempts (Table~\ref{tab:attacks}), and under a megabyte-class payload it is the slowest system in the table.
The kernel path reaches a zero bypass rate by placing the admission state somewhere no unprivileged process can reach; the seccomp baseline keeps filtering around the same mutable process that holds the decision, and the numbers reflect the difference.
...
None of these numbers are large enough to matter next to the cost of a single tool execution.
...
On the kernel path, a daemon restart is a reconciliation pass against state that was never in the daemon to begin with.
A forced restart mid-workflow is, in practice, a far more common event than any of the controlled attack attempts the security evaluation tests.

The cross-UID experiment in Section~\ref{sec:extended-attacks} is the clearest reminder that the zero-bypass table measures only design-time invariants: it exposed a peer-credential gap that the category-level harness did not probe, and that was closed by a six-line patch with no measurable latency cost.
The design-time and extended-attack layers are complementary, and neither on its own would have caught this.
