...
10\,KB & \textbf{0.863} & 0.922          & $+6.8\%$  & $2.1{\times}10^{-4}$  & 0.899          & $+4.2\%$    & $0.10^{\text{ns}}$ \\
\rowcolor{gray!8}
1\,MB  & 7.748          & 9.843          & $+27.0\%$ & $<10^{-4}$            & \textbf{7.487} & $-3.4\%$    & $2.7{\times}10^{-4}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Latency summary across the three systems at 100~B, 10~KiB, and 1~MiB payloads ($n = 30$ independent runs of 2000 requests, error bars 95\% CI). Panel (a): mean end-to-end latency on a log y-axis so sub-millisecond gaps at small payloads remain visible alongside the 1~MiB values. Panel (b): $p_{95}$ latency; the three systems are indistinguishable at 100~B and 10~KiB, and the kernel $p_{95}$ sits below both baselines at 1~MiB. Panel (c): mean-latency ratio against the \texttt{userspace} baseline, zoomed around the parity line; \texttt{seccomp} sits above parity at every payload size, while the kernel path only crosses below parity at 1~MiB. Panel (d): per-payload stage stack in $\mu$s, with three sub-axes auto-scaled per payload; each bar decomposes one system into session-lookup, arbitration, and tool-exec. The annotation above each kernel bar reports the absolute kernel arbitration cost (\num{28}~$\mu$s at 100~B and 10~KiB, \num{38}~$\mu$s at 1~MiB), which is the only stage with a meaningful inter-system delta and is two orders of magnitude smaller than tool execution at every payload size.}
\label{fig:chapter4-latency}
\end{figure}

The 1~MB row provides the only statistically clean comparison in the table, with the kernel path at \num{7.487}~ms against \num{7.748}~ms for plain user-space (a 3.4\% reduction, $p = 0.00027$) and \num{9.843}~ms for the hardened variant.
This is counter-intuitive: adding a privilege boundary is usually expected to cost latency, not return it.
Panel~(c) of Figure~\ref{fig:chapter4-latency} shows the same data as a ratio against the plain \texttt{userspace} baseline, with the y-axis zoomed around the parity line so that the sub-4\% effects at 100~B and 10~KB are visible in the same frame as the larger effect at 1~MB; read that way, \texttt{seccomp} sits above parity at every payload size while the kernel path crosses below parity only at 1~MB.
The explanation has two parts.
The seccomp penalty at large payloads is consistent with the accumulating cost of BPF per-syscall filtering across the heavier socket traffic that a megabyte payload forces through, a cost the kernel path does not incur. The modest speedup over plain user-space is consistent with arbitration happening before the payload is copied through the tool-forwarding socket, so a rejected request avoids a copy the user-space baseline would still have to perform between admission and forwarding.

The $p_{95}$ picture, shown in panel~(b) of Figure~\ref{fig:chapter4-latency}, matches the means.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.65\linewidth]{figures/figure_ablation_mean_rtt.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.65\linewidth]{figures/figure_ablation_stage_delta.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.65\linewidth]{figures/figure_registry_register_curve.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.65\linewidth]{figures/figure_registry_lookup_scaling.pdf}
...
\begin{table}[H]
\centering
\caption{Category-level attack outcomes by system (130 attempts each: 3 spoof $+$ 5 replay $+$ 3 substitution $+$ 2 escalation, $\times\,10$ repeats). ``Bypasses'' counts attempts that circumvented the intended control. 95\% Clopper--Pearson confidence intervals on the bypass rate are reported in the last column. User-space rates reflect the per-category instrumented weakening described in the text; they are \emph{not} the performance of a fully hardened user-space implementation. Per-case detail is plotted in Figure~\ref{fig:chapter4-security}.}
\label{tab:attacks}
...
We report this as an evaluation-discovered defect instead of burying it: the peer-credential check is exactly the kind of invariant a kernel-enforced control plane is supposed to hold, and the stock binary did not hold it until the E4 experiment ran.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.8\linewidth]{figures/figure_fuzz_errno_distribution.pdf}
...
The reconciliation strategy on restart is conservative (described in Section~\ref{sec:reconciliation}): cold start clears and fully rebuilds the tool registry from manifests, without attempting to reuse residual kernel state, even when the manifest set has not changed.
...
The security and performance results read together more clearly than apart. The seccomp baseline does strictly more work than the plain user-space mediator (tighter syscall filters, extra input validation, audit logging), yet still admits a substantial fraction of the attack attempts in Table~\ref{tab:attacks} and is the slowest system in Table~\ref{tab:latency} at megabyte-class payloads. The kernel path reaches a zero bypass rate by placing the admission state somewhere no unprivileged process can reach, while the seccomp baseline keeps filtering around the same mutable process that holds the decision.

The 1~MB latency inversion captures the same architecture on the performance side, since kernel arbitration runs on a fixed-size Generic Netlink message regardless of payload size while seccomp's BPF filter pays per-syscall. By the megabyte case, the stronger enforcement boundary is also the faster one. The ablation in Table~\ref{tab:ablation-stages} says the same thing from a different angle: of the \num{4.334}~$\mu$s that kernel arbitration adds over the NOOP floor, the registry+lookup stage costs \num{0.109}~$\mu$s and the rest is reply-skb construction. Nothing in the fast path scales with the number of tools or with the request body, and the registry scaling experiment extends this from microbench to catalogue size, showing that lookup latency stays within a \num{0.6}~$\mu$s band between $N = 8$ and $N = 16\,384$ tools and registration runs at roughly 166\,000 tools per second.

The durability result is less dramatic to present, but arguably the most practically important. A forced daemon restart mid-workflow is, in practice, a far more common event than any of the controlled attack attempts in the security evaluation, and the kernel path treats it as a reconciliation pass against state that was never in the daemon to begin with rather than as data loss.

The cross-UID experiment in Section~\ref{sec:extended-attacks} is the clearest reminder that the zero-bypass table measures only design-time invariants: it exposed a peer-credential gap that the category-level harness did not probe, and that was closed by a six-line patch with no measurable latency cost. The design-time and extended-attack layers are complementary, and neither on its own would have caught this.

Behaviour beyond the swept concurrency window is not characterised here. Inside the clean window up to 100 concurrent requests the kernel path stays within roughly 10\% of plain user-space throughput without diverging, but outside it, the global registry lock is an obvious suspect for a bottleneck. Per-tool or per-agent lock decomposition is the natural direction but sits outside the scope of this evaluation.
