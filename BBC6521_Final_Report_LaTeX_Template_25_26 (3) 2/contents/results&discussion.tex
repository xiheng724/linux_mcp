...
(These figures are drawn from an earlier $n = 5$ pilot run; the $n = 30$ study focused exclusively on latency and did not repeat the concurrency sweep. The throughput results should therefore be read as exploratory evidence of practical viability; the specific figures carry wider uncertainty than the latency results, and a definitive performance characterisation would require a dedicated concurrency sweep.)

The figures report mean throughput only; tail-latency evolution under sustained load (for instance $p_{99}$) would require a separate concurrency sweep and is not characterised here.
...
Looking at the three sets of results together, the security and performance numbers make more sense as a pair than separately.

The seccomp baseline is instructive here.
It does more work than the plain userspace baseline: tighter syscall filtering, extra validation, audit logging.
Yet it still permits 36.4\% of attack attempts.
The reason is that adding filters around a mutable user-space process does not move where the trust decision lives.
...
That is a qualitatively different property, and no amount of user-space hardening can substitute for it.

The 1~MB latency result is a side effect of the same architectural difference, which was not something anticipated going into the evaluation.
BPF filtering accumulates per-syscall cost as payload size grows; kernel arbitration does not scale with payload size.
So the seccomp baseline gets progressively slower on large payloads while the kernel path stays flat.
At 1~MB this produces a measurable inversion: the stronger enforcement boundary is actually faster.

The durability result is less dramatic to present but arguably the most practically important of the three.
When the daemon crashes, a user-space design loses everything it was holding: pending tickets, session context, registry state.
Recovery means starting over.
The kernel-path equivalent is just a daemon restart with reconciliation, because the state that matters was never in the daemon to begin with.
This is not an optimisation or a clever trick; it falls out automatically from where the state lives.
The scenario it addresses, a crash or forced restart mid-workflow, is probably more common in practice than the controlled attack attempts the security evaluation tests.

One thing the results do not settle is how the system behaves under heavier load.
The concurrency sweep went up to 100 concurrent requests and showed no throughput collapse, but the global registry lock is a real bottleneck waiting to be found.
At 500 or 1000 concurrent requests, particularly with many simultaneously deferred approvals, it could become a problem.
Per-tool or per-agent lock decomposition is the obvious direction, but it is not something the current prototype implements, and the evaluation does not characterise that regime.
