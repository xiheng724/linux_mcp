...
In the cross-UID experiment, \texttt{kernel\_mcp\_cmd\_tool\_request} did not compare \texttt{NETLINK\_CB(skb)} peer credentials against the UID recorded at agent registration, so requests from UID $B$ reached the handler as if they had originated from UID $A$.
Of the 500 attempts, 174 passed the checks enforced by the stock binary and would have been admitted to tool execution.
We fixed this with a six-line peer-credential check on entry to the handler, guarded by a \texttt{require\_peer\_cred} module parameter so the A/B comparison could use the same binary.
