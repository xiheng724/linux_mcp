...
This property---that approval tickets remain inspectable after daemon failure---is experimentally confirmed in Section~4.4.
...
This kernel--user-space partition---where the kernel owns registry state and admission decisions while user space retains semantic handling and tool execution---constitutes the central claim of this work. Chapter~4 experimentally validates that this split achieves meaningful security improvements without sacrificing compatibility or performance.
...
A capability-based design~\cite{lefeuvre2024sok} could scope each tool invocation to a capability derived from the tool's identity, preventing escalation across tool boundaries.
...
The cost is that it requires a loadable kernel module rather than a policy layer attached to existing hooks---a trade-off that is appropriate for a research prototype but would warrant careful security review before production deployment.
