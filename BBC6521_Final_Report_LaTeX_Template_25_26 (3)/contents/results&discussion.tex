...
The throughput figures do not characterise the distribution of tail latency under sustained load.
Mean per-second throughput is reported; a full account of how each system's $p_{99}$ evolves with concurrency would require a separate sweep that this dissertation does not attempt.
The results support the practical viability of the design.
Kernel arbitration adds overhead, but that overhead does not produce a throughput collapse at the concurrency levels tested.
Whether this holds at substantially higher concurrency or with much larger agent populations remains an open question; the current experiments do not go beyond 100 concurrent requests, and throughput at that point shows no signs of approaching a kernel-side bottleneck.


\subsection{Security Evaluation}

Performance is secondary to the question of whether kernel placement actually strengthens admission control.
The attack evaluation targets four categories corresponding to the threat model introduced in Chapter~2: identity spoofing (forging or borrowing session identity to impersonate a registered agent), replay (reusing a stale approval ticket across a new session or after expiry), substitution (tampering with the \texttt{tool\_id} or \texttt{tool\_hash} in a live request to redirect execution), and escalation (bypassing the approval gate so that a high-risk invocation proceeds without explicit confirmation).
...
Third, the user-space baseline figures require equally honest framing: those systems reach 90.9\% and 36.4\% bypass because the harness deliberately switches \texttt{mcpd} into instrumented tamper modes that relax specific checks, reflecting what a user-space mediator cannot structurally protect against when asked to validate its own state---not what a fully hardened user-space implementation would achieve.
...
Session state is not preserved---the binding context for active sessions is owned by the daemon process and is lost with it---but this is expected, since session state is inherently tied to an active connection.
...
The current reconciliation path is deliberately conservative: on startup, \texttt{mcpd} issues a \texttt{RESET\_TOOLS} command to clear the kernel-side tool registry, then re-registers the full set of tools from the current manifest.
This strategy rebuilds from the manifest source of truth rather than attempting to assess whether residual kernel state remains valid, keeping recovery logic straightforward and avoiding reasoning about partial updates.
An incremental approach---comparing kernel-held hashes against manifest-derived hashes and re-registering only tools whose hash has changed---would reduce restart overhead and is a natural direction for future work.


\subsection{Discussion}

Whether kernel placement is consequential depends on three separable questions: does it strengthen enforcement, does it impose prohibitive overhead, and does it provide durability that user-space designs cannot match?

The security evaluation provides the clearest answer.
The kernel path blocks all 110 attack attempts across four categories; neither user-space baseline does the same, even when augmented with stricter syscall filtering.
The reason is structural: approval tickets, agent bindings, and semantic hashes reside in kernel memory under \texttt{CAP\_NET\_ADMIN}-enforced access control, so a user-space process cannot modify or bypass them without a privilege escalation that lies outside the stated threat model.
The seccomp baseline's 36.4\% bypass rate illustrates precisely what user-space hardening cannot achieve: it narrows the syscall surface but leaves the admission decision and the state it depends in a mutable process.

The performance results demonstrate that this structural improvement does not carry a prohibitive cost.
The most informative comparison is at 1~MB: the kernel path is \emph{faster} than the seccomp baseline because BPF per-syscall filtering accumulates cost with payload size while kernel arbitration does not.
The Generic Netlink microbenchmark reinforces this: end-to-end arbitration adds approximately \num{1.1}~$\mu$s of lookup overhead above the bare round-trip, a cost that is invisible relative to tool execution at every payload size tested.

The failure experiment reveals a dimension that neither performance nor security benchmarks can capture: durability.
In both user-space baselines, daemon failure causes total loss of approval tickets and registry state.
In the kernel path, that state survives the failure and remains independently inspectable via \texttt{sysfs}.
This property does not emerge from a benchmark; it follows from holding state in the kernel rather than in a process-local data structure.
For long-running agent workflows, where failure and restart are routine rather than exceptional, this durability guarantee is likely to be as consequential as the security improvement.

The main open question concerns behavior at substantially higher concurrency.
The current experiments reach 100 concurrent requests without \emph{encountering} a kernel-side bottleneck, but the global registry lock and ticket-management logic have not been tested under workloads with many simultaneously deferred approvals or concurrency levels substantially beyond 100.
The ticket consume path serialises correctly under concurrent access, but whether the single global lock becomes a contention point at higher concurrency and whether per-tool or per-agent lock decomposition would reduce it, is still a concrete direction for follow-on work.
